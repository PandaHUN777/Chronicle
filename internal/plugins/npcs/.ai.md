...
The standalone gallery page is folded into the unified entities "Characters"
page: `GET /campaigns/:id/npcs` **redirects** to `/characters`, and the
...
There is no standalone gallery page template — that was removed once the
Index handler began redirecting.
...
| `handler.go` | HTTP handlers: Index redirect, reveal toggle, count API, `NPCSection` (Characters-page section), block data |
| `npc_section.templ` | `NPCSectionComponent` — the NPCs/Monsters section embedded in the Characters page (featured row + revealed list) |
...
|--------|------|---------|------|-------------|
| GET | `/campaigns/:id/npcs` | Index | Player+ | Redirects to `/campaigns/:id/characters` (gallery folded in) |
