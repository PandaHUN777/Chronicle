...
// Error-catalog parser and JSON artifact generator. This file walks
// errors.go via go/ast, extracts every constructor's Code and
// Category, and assembles a canonical structure that errors_test.go
// checks against both .ai.md's catalog table and the committed
// error-catalog.json artifact (regenerated by `make
// foundry-error-catalog`). If any of the three drift apart, the test
// fails with an actionable message naming which file to update.
