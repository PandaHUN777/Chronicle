...
func diagnosticCatalog() []Diagnostic {
	return []Diagnostic{
		...
		{
			Name:    "system.file-contains",
			Title:   "Check a served file for marker string(s)",
			Desc:    "Reads a served widget/manifest file (clamped to its system dir) and reports whether each comma-separated marker is present — confirms the live build's CONTENT, not just its hash.",
			ArgHint: "<system-id>:<relative-path>:<marker[,marker2]>",
		...
		},
		{
			...
			Title:   "Declared-field population across a type's entities",
			Desc:    "For one entity type, shows how many of its declared fields are actually non-empty across its entities (emptiest first). Surfaces 'declared but never populated' — the backfill/sync smell.",
			ArgHint: "<campaignId>:<entityTypeIdOrName>",
			Run:     renderFieldCoverage,
		},
		{
			Name:    "sync.inbound",
		...
		{
			Name:  "sync.recent",
			Title: "Recent inbound sync payloads across ALL entities",
			Desc:  "The last several inbound field payloads from external clients, newest first — a quick 'is anything syncing at all?' check.",
			Run:   func(string) string { return renderSyncRecent() },
		},
		{
			Name:  "probes",
